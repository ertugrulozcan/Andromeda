package com.aero.andromeda.slideup;
...
import static com.aero.andromeda.slideup.SlideUp.State.HIDDEN;
import static com.aero.andromeda.slideup.SlideUp.State.SHOWED;
...
public class SlideUp implements View.OnTouchListener, ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener, LoggerNotifier
{
	...
	private void init()
	{
		...
		if (mBuilder.mStartState == SHOWED)
			mCurrentState = SHOWED;
		if (mBuilder.mStartState == HIDDEN)
			mCurrentState = HIDDEN;
		
		mBuilder.mSliderView.getViewTreeObserver().addOnGlobalLayoutListener(new OnGlobalLayoutSingleListener(mBuilder.mSliderView, new Runnable()
		{
			@Override
			public void run()
			{
	...
	private void setTouchableAreaHorizontal()
	{
		if (mBuilder.mTouchableArea == 0)
		{
			...
			mBuilder.mTouchableArea = (float) Math.ceil(mViewWidth);
		}
	}
	...
	private void setTouchableAreaVertical()
	{
		if (mBuilder.mTouchableArea == 0)
		{
			...
			mBuilder.mTouchableArea = (float) Math.ceil(mViewHeight);
		}
	}
	...
	{
		((InputMethodManager) mBuilder.mSliderView.getContext().getSystemService(Context.INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(mBuilder.mSliderView.getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
	}
	...
	{
		((InputMethodManager) mBuilder.mSliderView.getContext().getSystemService(Context.INPUT_METHOD_SERVICE)).showSoftInput(mBuilder.mSliderView, 0);
	}
	...
	public void notifyPercentChanged(float percent)
	{
		...
		if (percent == 100)
		{
			...
			if (originalPercent == 100 && mCurrentState == SHOWED)
			{
		...
		else
		{
			...
			if (originalPercent == 0 && mCurrentState == HIDDEN)
			{
			...
			for (int i = 0; i < mBuilder.mListeners.size(); i++)
			{
				...
				if (l != null)
				{
					if (l instanceof Listener.Slide)
					{
						...
						slide.onSlide(percent, isShowAction || isHideAction);
	...
	public interface Listener
	{
		...
		interface Slide extends Listener
		{
			 ...
			 */
			void onSlide(float percent, boolean isShowOrHide);
		}
